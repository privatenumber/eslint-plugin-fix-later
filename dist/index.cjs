...
const insertCommentSameLine = (code, lineStart, comment) => {
  ...
  if (commentIndex) {
    ...
    if (commentBefore) {
      const insertAt2 = lineStart + commentIndex + commentBefore.index;
      const insertComment = parseEslintDisableComment(comment);
      return {
        range: [insertAt2, insertAt2],
        text: `, ${insertComment.rules} -- ${insertComment.description}`
      };
    }
  }
...
const suppressFileErrors = (code, sourceCode, extractedConfig, messages, { fix, filename }) => {
  ...
  for (const key in groupedByLine) {
    ...
    const insertCommentAbove = ruleOptions.insertDisableComment === "above-line" || groupedMessages.start.length > 0 || groupedMessages.end.length > 0;
    messages.push({
      /**
      ...
      line,
      column: 0,
      fix: insertCommentAbove ? insertCommentAboveLine(
        code,
        lineStartIndex,
        comment
