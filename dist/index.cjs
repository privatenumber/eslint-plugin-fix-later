...
const getDisableDirectiveIndex = (comment) => {
...
const parseEslintDisableComment = (comment) => {
  ...
  const index = description ? disableIndex + commentContent.indexOf("--") + 2 : disableIndex + commentContent.length;
  return {
    rules: rules.trim(),
...
const insertCommentAboveLine = (code, lineStart) => {
...
var require$1 = (
			false
				? /* @__PURE__ */ module$1.createRequire((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('index.cjs', document.baseURI).href)))
				: require
		);
...
const getVueElementNodeByRangeIndex = (index, rootNode) => {
...
const suppressFileErrors = (code, sourceCode, extractedConfig, messages, { fix, filename }) => {
  ...
  if (fix) {
    processMessages = processMessages.filter((message) => !message.fix || typeof fix === "function" && !fix(message));
  } else {
  ...
  const fixesMap = /* @__PURE__ */ new Map();
  const getFixMap = (insertAt, type) => {
  ...
  function isInJSXExpressionContainer(node) {
    let curr = node;
    while (curr) {
  ...
  for (const message of processMessages) {
    ...
    if (reportedNode) {
      ...
      const theFix = preferCommentAbove ? insertCommentAboveLine(
        code,
        lineStartIndex
  ...
  for (const [insertAt, fix2] of fixesMap) {
    const comments = [];
    if (fix2.enable.length > 0) {
      const rules = getRuleIds(fix2.enable).join(", ");
      comments.push(
        fix2.enable.text(`${commentSyntax[fix2.type][0]}eslint-enable ${rules}${commentSyntax[fix2.type][1]}`)
      );
    }
    if (fix2.disable.length > 0) {
      const [message] = fix2.disable;
      const rules = getRuleIds(fix2.disable).join(", ");
      comments.push(
        fix2.disable.text(`${commentSyntax[fix2.type][0]}eslint-disable ${rules} -- ${getLineComment(message)}${commentSyntax[fix2.type][1]}`)
      );
    }
    if (fix2["disable-next-line"].length > 0) {
      const [message] = fix2["disable-next-line"];
      const rules = getRuleIds(fix2["disable-next-line"]).join(", ");
      comments.push(
        fix2["disable-next-line"].text(`// eslint-disable-next-line ${rules} -- ${getLineComment(message)}`)
      );
    }
    if (fix2["disable-line"].length > 0) {
      const [message] = fix2["disable-line"];
      const rules = getRuleIds(fix2["disable-line"]).join(", ");
      comments.push(
        fix2["disable-line"].text(`// eslint-disable-line ${rules} -- ${getLineComment(message)}`)
      );
    }
    const fixObject = {
